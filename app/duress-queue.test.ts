...
describe('duress-queue', () => {
  it('queues and drains alerts for a group', () => {
    ...
    const drained = drainDuressQueue('group1')
    expect(drained).toHaveLength(2)
    expect(drained[0].opId).toBe('1')
    expect(drained[1].opId).toBe('2')

    // Queue is empty after drain
    expect(drainDuressQueue('group1')).toHaveLength(0)
  })

  it('only drains alerts for the specified group', () => {
    ...
    const drained2 = drainDuressQueue('group2')
    expect(drained2).toHaveLength(1)
    expect(drained2[0].opId).toBe('b')
  })

  it('clearDuressQueue removes everything', () => {
