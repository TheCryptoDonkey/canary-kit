...
This rotates the group secret immediately. Remaining members must re-join using a fresh invite.`))return;const{activeGroupId:f}=x();if(!f)return;Rv(f,l);const{groups:h}=x(),p=h[f];p&&p.members.length>0&&Os(p)}),e.querySelector("#invite-btn")?.addEventListener("click",()=>{const{groups:a,activeGroupId:c}=x();if(!c)return;const l=a[c];l&&Oo(l)}),e.querySelector("#share-state-btn")?.addEventListener("click",()=>{const{groups:a,activeGroupId:c}=x();if(!c)return;const l=a[c];l&&Os(l)}),e.querySelector("#confirm-member-btn")?.addEventListener("click",()=>{$u()})}function rc(e,t,n){const{groups:r,identity:o}=x(),s=r[e];if(!s||!o?.pubkey||!s.admins.includes(o.pubkey))return!1;s.members.includes(t)||Ns(e,t,n);const i=x().groups[e];return i&&n&&J(e,{memberNames:{...i.memberNames,[t]:n}}),!0}function $u(e){const{groups:t,activeGroupId:n}=x();!n||!t[n]||(Oi(`
